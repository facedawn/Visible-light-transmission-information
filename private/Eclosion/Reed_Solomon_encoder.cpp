...
GenericGFPoly *RSencoder::buildGenerator(int d)
{
	using namespace std;
	if (d > cacheGen.size()) {
		GenericGFPoly* lastGen = cacheGen[cacheGen.size() - 1];
		for (int i = cacheGen.size(); i <= d; i++) {
			GenericGFPoly* nextGen = lastGen->multiply(new GenericGFPoly(field, std::vector<int>{1,field->exp(i-1+field->getGeneBase())}));
...
RSencoder::RSencoder(GenericGF *field)
{
	this->field = field;
	this->cacheGen.push_back(new GenericGFPoly(field, std::vector<int>{1}));
}

void RSencoder::encode(std::vector<int>& E, int ecBytes)
{
