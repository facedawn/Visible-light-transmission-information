...
GenericGFPoly::GenericGFPoly(GenericGF *field, std::vector<int>& coef)
{
	...
	else this->coeff = coef;
}

GenericGFPoly *GenericGFPoly::getZero()
{
	return new GenericGFPoly(this->field, std::vector<int>{0});
}

GenericGFPoly *GenericGFPoly::getOne()
{
	return new GenericGFPoly(this->field, std::vector<int>{1});
}

int GenericGFPoly::evaluateAt(const int a)
{
	if (a == 0) return getCoeff(0);
	int size = coeff.size();
	if (a == 1)
    {
		int result = 0;
...
GenericGFPoly *GenericGFPoly::add_or_sub(GenericGFPoly * other)
{
...
GenericGFPoly *GenericGFPoly::multiply(const int scalar)
{
...
GenericGFPoly *GenericGFPoly::multiply(GenericGFPoly *other)
{
	assert(!(typeid(field) == typeid(other)) && "GenericGFPolys do not have same GenericGF field !!!");
	...
	for (int i = 0; i < asize; i++)
	{
		...
		for (int j = 0; j < bsize; j++) {
			product[i + j] = GenericGF::add_or_sub(product[i + j], field->multiply(ac, bcoeff[j]));
		}
	}
	return new GenericGFPoly(field, product);
}

GenericGFPoly *GenericGFPoly::multiply_by_monomial(const int d, const int c)
{
...
std::vector<GenericGFPoly*>GenericGFPoly::divide(GenericGFPoly * other)
{
	assert(!(typeid(field) == typeid(other)) && "GenericGFPolys do not have same GenericGF field !!!");
...
GenericGFPoly * GenericGFPoly::buildMonomial(int d, int c)
{
	assert(!(d < 0) && "Illegal Argument Exception!!!");
...
std::string GenericGFPoly::to_string()
{
	std::string str = "";
